...
if (have_comments()) {
	if (!empty($comments_by_type['comment'])) {
		...
		($comment_count !== 1) ? $comment_text = __('コメント', 'mh-magazine-lite') : $comment_text = __('コメント', 'mh-magazine-lite'); ?>
...
if (comments_open()) {
	$custom_args = array(
    	'title_reply' => __('コメントする', 'mh-magazine-lite'),
        'comment_notes_before' => '<p class="comment-notes">' . __('', 'mh-magazine-lite') . '</p>',
        'comment_notes_after'  => '',
        'comment_field' => '<p class="comment-form-comment"><label for="comment">' . __('コメント', 'mh-magazine-lite') . '</label><br/><textarea id="comment" name="comment" cols="45" rows="5" aria-required="true"></textarea></p>');
