...
if (!function_exists('mh_scripts')) {
	function mh_scripts() {
		...
		wp_enqueue_style('mh-style', get_stylesheet_uri(), false, '1.7.0');
...
if (!function_exists('mh_content_css')) {
	function mh_content_css() {
		$mh_magazine_lite_options = mh_magazine_lite_theme_options();
		if (isset($mh_magazine_lite_options['sb_position']) && $mh_magazine_lite_options['sb_position'] == 'left') {
			$float = 'right';
		} else {
			$float = 'left';
		}
...
if (!function_exists('mh_wp_title')) {
	function mh_wp_title($title, $sep) {
		...
		if ($paged >= 2 || $page >= 2)
			$title = "$title $sep " . sprintf(__('Page %s', 'mh-magazine-lite'), max($paged, $page));
		return $title;
	}
}
...
if (!function_exists('mh_featured_image')) {
	function mh_featured_image() {
		...
		if (has_post_thumbnail() && !is_attachment()) {
			...
			if (isset($mh_magazine_lite_options['no_prettyphoto']) ? !$mh_magazine_lite_options['no_prettyphoto'] : true) {
		...
		if ($ab_output) {
			...
			echo '<section class="author-box">' . "\n";
				echo '<div class="author-box-wrap clearfix">' . "\n";
					...
					echo '<h5 class="author-box-name">' . __('About ', 'mh-magazine-lite') . esc_attr($name) . '</h5>' . "\n";
					...
					echo '<div class="post-nav left">' . "\n";
						if ($attachment) {
							if ($count == 1) {
								...
								echo '<a href="' . $permalink . '">' . __('&larr; Back to article', 'mh-magazine-lite') . '</a>';
							} else {
...
if (!function_exists('mh_related')) {
	function mh_related() {
		global $post;
		$mh_magazine_lite_options = mh_magazine_lite_theme_options();
		if (isset($mh_magazine_lite_options['related_posts']) ? $mh_magazine_lite_options['related_posts'] : false) {
			...
			if ($tags) {
				...
				if ($related->have_posts()) {
					...
					echo '<h3 class="section-title">' . __('Related Articles', 'mh-magazine-lite') . '</h3>' . "\n";
